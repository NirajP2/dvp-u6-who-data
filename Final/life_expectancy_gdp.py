...
#import pyplot, pandas and seaborn
from matplotlib import pyplot as plt
import pandas as pd
import seaborn as sns
...
#read csv file and create dataframe using pandas and store it in variable df
...
# Chile, China, Germany, Mexico, United State of America, Zimbabwe

# What years are represented in the data?

# 2000 through to 2015
...
#renaming of columns involves inputting a dictionary with the key as the original column name and the 
#value as the desired name of the column. The dataframe has been modified in place for simplicity.
...
#Seaborn barplot function used to generate the bar plot. 
sns.barplot(data=df, x="Country", y="GDP")
plt.xticks(rotation=45)   # x-ticks have been rotated for clarity
plt.show()
...
# No. The life expectancy of Chile, China, Germany, Mexico and USA are all relatively similar, however the GDP for
# these countries varies considerably. This is because GDP needs to be normalised against population. Zimbabwe is different to the first five countries.
...
plt.ylabel("Life expectancy at birth (Years)") # clearer labelling for blog
plt.savefig("violin_plot.png") # save plot as png image for blog
plt.show()


# What do you notice about this distribution? Which country's life expactancy has changed the most?

#  The first five countries have "short" distributions i.e. they do not vary considerably over the 15 year dataset. Zimbabwe's improves considerably over the 15 year span.
...
# 
# 1) Which countries' bars changes the most?
# 2) What years are there the biggest changes in the data?
# 3) Which country has had the least change in GDP over time? 
# 4) How do countries compare to one another?
# 5) Now that you can see the both bar charts, what do you think about the relationship between GDP and life expectancy?
# 6) Can you think of any reasons that the data looks like this for particular countries?

# 1) China and USA have the largest change in GDP, whilst Zimbabwe has the largest change in life expectancy.
# 
# 2) For Chinese GDP, from late 2008/9 through to 2015. For USA GDP, from 2000 through to 2015. For Zimbabwe's life expectancy, from 2006/7 through to 2015.
# 
# 3) Zimbabwe's GDP does not appear to have changed much over time, however this may be because the smaller numbers mean that the dataset is effectively invisible. Chile's GDP has also not changed significantly relative to the other countries.
# 
# 4) GDP varies considerably across the nations. China and the US have the largest increases in GDP. Life expectancy is similar for the first five countries and increases slowly over time. Zimbabwe has the largest increase in life expectancy.
# 
# 5) Life expectancy does not seem to vary with GDP unless GDP is very low.
# 
# 6) China and Zimbabwe have experienced large growth over the start of the 21st century. However, for the other countries, growth appears to be consistent.
# 
...
g = sns.FacetGrid(df, col="Year", hue="Country", palette="Accent", col_wrap=4, size=2)
...
# China, but USA close second.
# Zimbabwe.
# Not surprising, please see all the previous responses.
...
# Highest: USA and eventually China, however in the early 2000's Germany had a higher GDP than China; Lowest: Zimbabwe and Chile

# Which countries have the highest and lowest life expectancy?

# Highest: USA and Germany, Lowest: Zimbabwe and China
...
# Why does the life expectancy in Zimbabwe drop for the first few years of the dataset?
# The following sources of information have been used to answer the above question:
# https://www.medicalnewstoday.com/articles/41339.php
# https://www.researchgate.net/publication/297750732_Life_Expectancy_In_Zimbabwe_An_Analysis_Of_Five_Decades
# http://www.chronicle.co.zw/zim-life-expectancy-shoots-up/
# 
# The drop in life expectancy in Zimbabwe from 2000 through to 2004 is a continuation of a drop in life expectancy that started in the 1990's. The drop in life expectancy is attributed to the deteriorating economic conditions and a HIV epidemic.
# The improvement in life expectancy is due to the improved treatment of HIV and AIDS, especially in the prevention of transmission in childbirth.
